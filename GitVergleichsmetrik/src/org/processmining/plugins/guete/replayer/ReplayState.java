...
public class ReplayState<C extends ReplayCost & Comparable<? super C>> implements Comparable<ReplayState<C>> {
	 ...
	 */
	public ReplayState<C> parentState;
	...
	public int compareTo(ReplayState<C> state) {
		...
		/*
		 * If costs are equal, prefer shorter traces.
		 */
		c = trace.size() - state.trace.size();
		if (c != 0) {
			return c;
		}
		/*
		 * Costs and trace sizes are equal. Enforce total order using unique id.
		 */
